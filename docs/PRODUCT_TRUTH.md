...
|---|---|---|
| World size | **5 stations / 5 playable exercises**, 923 instances / 854 BaseParts, build `ed9b6b379389`; 15 original machine and pose definitions remain configured for future updates | `scripts/validate_gym.py` |
| World source | generated by `scripts/build_gym.py` into `src/Workspace/Gym/*.model.json` | repo layout |
| Zones | 11 definitions remain for future progression, but all five current destinations use the open-from-spawn Garage rate | `ZoneConfig.luau`, generated GymZone attributes |
...
| Persistent navigation | one right-side **MENU** button plus a bottom five-stat training dock; pressing a stat opens its Train view | `MenuBarController.luau`, `HudController.luau`, `MenuController:OpenTrain` |
| Finding a machine | the **Train** tab lists the one current location for the chosen muscle with its authored place name and live free/busy count | `MenuController:_renderTrain`, `TrainingService:GetSpotStatus` |
...
| Machine visibility | any of the five stations within 260 studs carries a muscle-coloured outline and floor ring | `StationHighlightController` |
...
4. **The current playable slice is five locations, not 55.** Each stat has one exercise:
   Dumbbells, Bench Press, Pull-Up Bar, Sit-Up Bench, and Treadmill. The city deliberately
   varies access—safe plaza, warehouse, third floor, underpass, and flight-only sky deck.
   The ten extra configured exercises are dormant content, not duplicated world stations.
